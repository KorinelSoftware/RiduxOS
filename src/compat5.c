...
/* 3. arc4random (ChaCha20, OpenBSD/FreeBSD adapted) */
...
static void c5ai_resolv_defaults(c5ai_resolv_conf_t *rc){
    ...
    rc->ndots=1;
    rc->nameservers[0]=((uint32_t)10u<<24)|((uint32_t)0u<<16)|((uint32_t)2u<<8)|3u; /* 10.0.2.3 */
...
/* 9. Browser-specific syscall stubs */
...
static int c5_spawn_elf_task(const char *path,bool launch_now,char *detail,size_t cap){
    ...
    t->aux_at_egid=(uint64_t)t->egid;
    {
        ...
        is_vulkan_probe=c5_has_token_ci(path,"ridux-vulkan-probe");
        if(is_chrome||is_firefox){
            char app_dir[VFS_PATH_MAX];
            /*
             * Keep normal user-mode timer preemption for browser threads.
             * Disabling it avoided one old heap-corruption path, but once the
             * syscall frame is per-task it can also let a CPU-bound browser
             * worker monopolize the single CPU before GTK reaches the first
             * real top-level map/paint.
             */
        ...
        if(is_chrome){
            ...
            argv_exec[argc_exec++]="--ignore-gpu-blocklist";
            argv_exec[argc_exec++]="--enable-gpu-rasterization";
            argv_exec[argc_exec++]="--enable-zero-copy";
            argv_exec[argc_exec++]="--enable-accelerated-2d-canvas";
            argv_exec[argc_exec++]="--enable-accelerated-video-decode";
            argv_exec[argc_exec++]="--enable-native-gpu-memory-buffers";
            argv_exec[argc_exec++]="--use-gl=egl";
            argv_exec[argc_exec++]="--disable-dev-shm-usage";
            argv_exec[argc_exec++]="--no-zygote";
            argv_exec[argc_exec++]="--single-process";
            ...
            argv_exec[argc_exec++]="--metrics-recording-only";
            argv_exec[argc_exec++]="--disable-features=MojoUseEventFd,CalculateNativeWinOcclusion,MediaRouter,DialMediaRouteProvider,OptimizationHints,AutofillServerCommunication,CertificateTransparencyComponentUpdater,FirstRunDesktopRevamp,FirstRunDesktopRefresh,SitePerProcess,IsolateOrigins,StrictOriginIsolation,ProcessPerSiteUpToMainFrameThreshold";
            ...
            argv_exec[argc_exec++]="--enable-features=UseOzonePlatform,Vulkan,CanvasOopRasterization,VaapiVideoDecoder";
            argv_exec[argc_exec++]="--ozone-platform=x11";
            argv_exec[argc_exec++]="about:blank";
        }else if(is_firefox){
        ...
        if(is_chrome){
            ...
            /* Firefox renders fine on plain X11 without a session bus.  Our
             * virtual DBus is still incomplete enough that GTK/GIO can wait
             * there before Firefox ever maps its first window, so fail DBus
             * discovery quickly and keep settings local. */
            env_exec[envc++]="XDG_SESSION_TYPE=x11";
            env_exec[envc++]="XDG_CONFIG_HOME=/tmp";
            env_exec[envc++]="XDG_CACHE_HOME=/tmp";
            ...
            env_exec[envc++]="MOZ_APP_LAUNCHER=/opt/firefox/firefox-bin";
            env_exec[envc++]="DBUS_SESSION_BUS_ADDRESS=unix:path=/tmp/ridux-no-dbus";
            env_exec[envc++]="DBUS_SYSTEM_BUS_ADDRESS=unix:path=/tmp/ridux-no-system-dbus";
            env_exec[envc++]="GTK_USE_PORTAL=0";
            env_exec[envc++]="GSETTINGS_BACKEND=memory";
        ...
        if(is_vulkan_probe){
            env_exec[envc++]="XDG_SESSION_TYPE=tty";
            env_exec[envc++]="XDG_CURRENT_DESKTOP=Ridux";
            env_exec[envc++]="DESKTOP_SESSION=ridux-vulkan";
            env_exec[envc++]="RIDUX_VULKAN_PROBE=1";
            env_exec[envc++]="LD_PRELOAD=/lib64/ridux-glibc-private-shim.so";
            env_exec[envc++]="LD_BIND_NOW=1";
            env_exec[envc++]="LIBGL_ALWAYS_SOFTWARE=0";
            env_exec[envc++]="MESA_LOADER_DRIVER_OVERRIDE=virtio_gpu";
            env_exec[envc++]="GALLIUM_DRIVER=virgl";
            env_exec[envc++]="MESA_DEBUG=context";
            env_exec[envc++]="MESA_LOG_LEVEL=warn";
            env_exec[envc++]="MESA_LOG_FILE=/dev/stderr";
            env_exec[envc++]="VN_DEBUG=warn";
            env_exec[envc++]="MESA_VK_ABORT_ON_DEVICE_LOSS=0";
            env_exec[envc++]="VK_LOADER_DEBUG=error,warn";
            env_exec[envc++]="VK_DRIVER_FILES=/usr/share/vulkan/icd.d/virtio_icd.x86_64.json";
            env_exec[envc++]="VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/virtio_icd.x86_64.json";
            env_exec[envc++]="VK_LOADER_DRIVERS_SELECT=virtio_icd.x86_64.json";
        }
        if(is_chrome||is_firefox){
            ...
            env_exec[envc++]="__GLX_VENDOR_LIBRARY_NAME=mesa";
            env_exec[envc++]="LIBGL_ALWAYS_SOFTWARE=0";
            env_exec[envc++]="MESA_SHADER_CACHE_DIR=/tmp/mesa-shader-cache";
            env_exec[envc++]="MESA_SHADER_CACHE_DISABLE=0";
            env_exec[envc++]="VK_DRIVER_FILES=/usr/share/vulkan/icd.d/intel_icd.x86_64.json:/usr/share/vulkan/icd.d/intel_hasvk_icd.x86_64.json:/usr/share/vulkan/icd.d/radeon_icd.x86_64.json:/usr/share/vulkan/icd.d/nouveau_icd.x86_64.json:/usr/share/vulkan/icd.d/virtio_icd.x86_64.json:/usr/share/vulkan/icd.d/vmwgfx_icd.x86_64.json";
            env_exec[envc++]="VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/intel_icd.x86_64.json:/usr/share/vulkan/icd.d/intel_hasvk_icd.x86_64.json:/usr/share/vulkan/icd.d/radeon_icd.x86_64.json:/usr/share/vulkan/icd.d/nouveau_icd.x86_64.json:/usr/share/vulkan/icd.d/virtio_icd.x86_64.json:/usr/share/vulkan/icd.d/vmwgfx_icd.x86_64.json";
        }
        if(is_firefox){
            env_exec[envc++]="MOZ_WEBRENDER=1";
            env_exec[envc++]="MOZ_ACCELERATED=1";
            env_exec[envc++]="MOZ_X11_EGL=1";
            env_exec[envc++]="MOZ_ENABLE_WAYLAND=0";
            env_exec[envc++]="MOZ_USE_XINPUT2=0";
            env_exec[envc++]="GDK_BACKEND=x11";
            env_exec[envc++]="GDK_GL=gles";
            ...
            env_exec[envc++]="MOZ_DISABLE_E10S=1";
            env_exec[envc++]="MOZ_FORCE_DISABLE_E10S=1";
            env_exec[envc++]="MOZ_DISABLE_NONLOCAL_CONNECTIONS=1";
...
static void cmd5_browser(const char*a,char*o,int mx){
    ...
    if(c5_next_tok(&argp,sub,sizeof(sub))){
        ...
        if(ulibc_strcmp(sub,"run")==0||ulibc_strcmp(sub,"realrun")==0){
            ...
            /* IMPORTANT: open the native UI BEFORE c5_spawn_elf_task when
             * launch_now is true. task_launch_to_user() never returns
             * once it iretq's into ring 3, so any UI work queued after
             * the spawn would never execute. Opening the window first
             * (and letting the caller's render path paint it) guarantees
             * the user sees Firefox/Chromium even if the real binary
             * exits silently, panics in glibc, or fails to connect to
             * the Wayland compositor. */
            if(launch_now){
                if(c5_has_token_ci(exec_path,"firefox")||c5_has_token_ci(run_path,"firefox")){
                    compat_ui_open_app("firefox");
                }else{
